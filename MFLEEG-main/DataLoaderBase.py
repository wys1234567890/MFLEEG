...
class ArrayDataLoader(Dataset):
    def __init__(self, dataset):
        super().__init__()
        """
        最基础的数据集加载类，直接加载单组数据 + 标签的共享数组，适用于测试集或单个客户端的数据集加载
...
class SplitTrain(Dataset):
    def __init__(self, dataset, test_index, Common_config):
        super().__init__()
        """
        从多被试数据中拆分出训练集，支持两种拆分策略：
        subject_wise=True：按「被试级别」拆分（部分被试全量数据作为训练）；
        subject_wise=False：按「试次级别」拆分（所有剩余被试数据合并后，按比例拆分试次）。
        ...
        if Common_config["subject_wise"]:
            random.seed(42)
            # 随机选取 (1 - split_ratio) 比例的被试作为训练被试
