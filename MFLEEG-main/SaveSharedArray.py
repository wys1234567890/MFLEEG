...
# 将多客户端、多被试的 EEG（脑电）信号数据加载到多进程共享数组中，适配 PyTorch 的张量格式
# 为多进程并行处理（如分布式训练、多客户端数据加载）提供数据共享能力
...
def Load_data_into_shared_array_allClient(Common_config, Client_config_list):
    """
    遍历所有客户端，为每个客户端加载其下所有被试的共享数组数据，最终返回多客户端的数据集列表；
...
def Target_client(Common_config, TargetC_config):
    """
    为单个目标客户端加载所有被试的 EEG 数据，将每个被试的数据和标签转为多进程共享数组，并封装为 PyTorch 张量
